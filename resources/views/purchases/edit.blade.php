                            ...
                            <div class="row">
                                ...
                                <div class="col-12">
                                    <table class="table table-bordered" id="productTable">
                                        ...
                                        <tbody>
                                            @if(!empty($purchase->stocks ))
                                                @foreach ($purchase->stocks  as $index => $product)
                                                <tr>
                                                    <td>
                                                    ...
                                                    <td><input type="number" name="products[{{ $index }}][pack]" value="{{ $product->pack }}" class="form-control"></td>
                                                    <td><input type="text" name="products[{{ $index }}][batch]" value="{{ $product->batch }}" class="form-control"></td>
                                                    <td><input type="date" name="products[{{ $index }}][expiry_date]" value="{{ $product->expiry_date }}" class="form-control"></td>
                                                    <td><input type="number" name="products[{{ $index }}][quantity]" value="{{ $product->quantity }}" class="form-control"></td>
                                                    <td><input type="number" name="products[{{ $index }}][bonus]" value="{{ $product->bonus }}" class="form-control"></td>
                                                    <td><input type="text" name="products[{{ $index }}][rate]" value="{{ $product->rate }}" class="form-control"></td>
                                                    <td><input type="number" step="0.1" min="0" name="products[{{ $index }}][cc]" value="{{ $product->cc }}" class="form-control"></td>
                                                    <td><input type="text" name="products[{{ $index }}][cc_on_bonus]" value="{{ $product->cc_on_bonus }}" class="form-control"></td>
                                                    <td><input type="text" name="products[{{ $index }}][amount]" value="{{ $product->quantity * $product->rate + $product->cc_on_bonus }}" class="form-control"></td>
                                                    <td><input type="text" name="products[{{ $index }}][sp]" value="{{ $product->selling_price }}"  class="form-control"></td>
                                ...
                                <div class="col-md-12 d-flex justify-content-between">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="d-flex align-items-center mb-3">
                                            ...
                                            <input type="text" class="form-control" name="net_amount" value="{{ old('net_amount', $purchase->net_amount) }}" placeholder="Enter net amount" id="netamountInput">
                                        </div>

                                        <!-- Discount -->
                                        <div class="d-flex align-items-center mb-3">
                                            <label for="discountNumber" class="form-label me-2" style="min-width: 120px;">Discount</label>
                                            <input type="text" class="form-control" name="discount" value="{{ old('discount', $purchase->discount) }}" placeholder="Enter discount amount" id="discountNumber">
                                        ...
                                        <div class="d-flex align-items-center mb-3">
                                            <div class="d-flex align-items-center me-3" style="min-width: 110px;">
                                                <input class="form-check-input me-2" type="checkbox" id="vatToggle" {{ old('vat', $purchase->vat) ? 'checked' : '' }}>
                                                <label class="form-check-label mb-0" for="vatToggle">Apply VAT</label>
                                            </div>
                                            <input type="text" class="form-control" name="vat" value="{{ old('vat', $purchase->vat) }}" id="vatInput">
                                        </div>

                                        <!-- Total Amount -->
                                        <div class="d-flex align-items-center mb-3">
                                            <label for="totalAmount" class="form-label me-2" style="min-width: 120px;">Total Amount</label>
                                            <input type="text" class="form-control" name="total_amount" value="{{ old('total_amount', $purchase->total_amount) }}" placeholder="Enter total amount" id="totalAmount">
                                        </div>
                                    </div>
                                </div>
